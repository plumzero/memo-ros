...
/*---------------------------------------------*/
static __inline__ void fallbackSimpleSort(UInt32* fmap, UInt32* eclass,
                                          Int32 lo, Int32 hi) {
  Int32 i, j, tmp;
  UInt32 ec_tmp;
...
static void fallbackSort(UInt32* fmap, UInt32* eclass, UInt32* bhtab,
  ...
  H = 1;
  while (1) {
    if (verb >= 4) VPrintf1("        depth %6d has ", H);
    ...
    if (verb >= 4) VPrintf1("%6d unresolved strings\n", nNotDone);
  ...
  --*/
  if (verb >= 4) VPrintf0("        reconstructing block ...\n");
  j = 0;
  for (i = 0; i < nblock; i++) {
...
/*---------------------------------------------*/
static __inline__ Bool mainGtU(UInt32 i1, UInt32 i2, UChar* block,
      ...
      if (j != ss) {
        ...
        if (!(ftab[sb] & SETMASK)) {
          ...
          if (hi > lo) {
            ...
            mainQSort3(ptr, block, quadrant, nblock, lo, hi, BZ_N_RADIX,
                       budget);
            numQSorted += (hi - lo + 1);
            if (*budget < 0) return;
          }
