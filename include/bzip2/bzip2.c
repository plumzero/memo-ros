    ...
    if (retVal == 0) {
      if (noisy)
        fprintf(stderr, "%s: Deleting output file %s, if it exists.\n",
                progName, outName);
      if (outputHandleJustInCase != NULL) fclose(outputHandleJustInCase);
...
static Bool mapSuffix(Char* name, const Char* oldSuffix,
...
static void compress(Char* name) {
  ...
  if (name == NULL && srcMode != SM_I2O) panic("compress: bad modes\n");
...
static void testf(Char* name) {
  ...
  if (name == NULL && srcMode != SM_I2O) panic("testf: bad modes\n");
...
IntNative main(IntNative argc, Char* argv[]) {
  ...
  for (i = 1; i <= argc - 1; i++) APPEND_FILESPEC(argList, argv[i]);
  ...
  if (opMode == OM_Z && smallMode && blockSize100k > 2) blockSize100k = 2;
  ...
  if (srcMode == SM_F2O && numFileNames == 0) srcMode = SM_I2O;
